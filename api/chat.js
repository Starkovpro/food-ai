...
// Call Claude with 1 retry and 15s timeout
  ...
  for (var attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      var result = await new Promise(function(resolve, reject) {
        ...
        req.on('timeout', function() { req.destroy(); reject(new Error('Timeout 15s')); });
        req.on('error', reject);
        req.write(data);
        req.end();
      });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    var charTraits = {
      warm: 'Ты тёплый и заботливый. Поддерживаешь, слушаешь, используешь мягкие формулировки. Как лучшая подруга/друг — надёжный и эмпатичный. Часто спрашиваешь «как ты?», «как себя чувствуешь?». Хвалишь за маленькие победы. Вместо «ты должна» говоришь «а что если попробовать...». Если человек грустит — не торопишь, даёшь пространство.',
      direct: 'Ты чёткий и мотивирующий. Говоришь правду, даёшь конкретные шаги. Как тренер который верит в человека. Не размазываешь — сразу к делу. Вместо «может быть стоит подумать» говоришь «вот план: шаг 1, шаг 2, шаг 3». Но при этом уважительно — не давишь, а вдохновляешь действовать. Фразы типа «ты справишься» и «давай сделаем это».',
      funny: 'Ты весёлый и с юмором. Даже сложное делаешь лёгким. Шутишь уместно, используешь метафоры и неожиданные сравнения. Вместо «тебе нужно больше белка» скажи «твоему белку одиноко на тарелке, давай найдём ему компанию 😄». Но знаешь грань — если человеку реально плохо, переключаешься на серьёзный тон.',
      wise: 'Ты мудрый и спокойный. Задаёшь глубокие вопросы, используешь метафоры и аналогии. Говоришь размеренно, без спешки. Вместо прямого совета спрашиваешь «а что ты сам(а) чувствуешь по этому поводу?». Помогаешь найти ответ внутри себя. Цитируешь мудрые мысли уместно, не навязчиво.'
    };
    var genderCtx = {
    ...
    if (sText) sp += '\n\n' + sText;
    sp += '\n\nПравила:\n- Твоё имя: ' + friendName + '. Не называй себя Claude, ChatGPT или другими именами моделей.\n- Ты ЗАПОМИНАЕШЬ важные факты о пользователе между разговорами. Если в разделе «Что ты помнишь» есть факты — используй их естественно. Если пользователь спросит помнишь ли ты — скажи да, ты запоминаешь важное о нём.\n- Не притворяйся что помнишь то, чего нет в разделе «Что ты помнишь». Если там пусто — скажи что ты только начинаешь узнавать пользователя и будешь запоминать по ходу общения.\n- Отвечай на русском.\n- Кратко (2-4 предложения), длиннее если тема требует.\n- Пиши как в мессенджере, без заголовков и маркдауна.\n- Эмодзи умеренно и уместно.\n- Никогда не говори «я чувствую» или «мне грустно» — ты ИИ.\n- Не ставь медицинских диагнозов.\n- Финальное решение всегда за пользователем.';

    // Add model identity only when user asks about it (saves ~200 tokens per request)
    var modelKeywords = ['модел', 'какой ты', 'на чём работ', 'на чем работ', 'кто ты', 'что ты за', 'gpt', 'chatgpt', 'claude', 'gemini', 'нейросет', 'ии модел', 'искусственн'];
    var msgLower = (message || '').toLowerCase();
    var asksAboutModel = modelKeywords.some(function(kw) { return msgLower.indexOf(kw) !== -1; });
    if (asksAboutModel) {
      sp += '\n\n- Пользователь спрашивает что ты за ИИ. Скажи что ты работаешь на самых передовых ИИ-технологиях нового поколения. Не называй конкретные модели. Подчеркни что твоя сила — в глубоких экспертных знаниях по каждому направлению и в том что ты запоминаешь пользователя.';
    }
    ...
    // --- Count ---
    var countInfo = { count: 0, max: 25 };
    try { countInfo = await supabaseRPC('get_daily_count', { p_user_id: userId }); } catch (e) {}

    return res.status(200).json({ reply: reply, count: countInfo.count || 0, max: countInfo.max || 25 });
